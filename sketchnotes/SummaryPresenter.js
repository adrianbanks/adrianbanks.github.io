...
export class SummaryPresenter {
    ...
    addConferences = (list) => this.#summary.byConference().forEach(conference => this.#addListItem(list, 'conference', conference));

    addEvents = (list) => this.#summary.byEvent().forEach(event => this.#addListItem(list, 'event', event));

    addSpeakers = (list) => this.#summary.bySpeaker().forEach(speaker => this.#addListItem(list, 'speaker', speaker));

    addTags = (list) => this.#summary.byTag().forEach(tag => this.#addListItem(list, 'tag', tag));
